...
import { auditLog, drivers, fuelRecord, transportOperation } from '@/lib/db/schema'
...
export async function createOperation(input: { operationDate: string; truckCode: string; driverName: string; city: string; km: number; tons: number; liters: number; truckId?: number; driverId?: number; trips?: number; notes?: string }) {
  ...
  if (!input.operationDate || !input.truckCode.trim() || !input.driverName.trim() || !input.city.trim()) {
    throw new Error('Data, caminhão, motorista e destino são obrigatórios.')
  }
  await db.insert(transportOperation).values({
  ...
  await db.insert(auditLog).values({ userId: currentUser.id, action: 'create', entity: 'transport_operation', metadata: JSON.stringify(input) })
  revalidatePath('/')
  return created
}
...
export async function createDailyOperation(input: { truckId: number; driverName: string; operationDate: string; km: number; trips: number; tons: number; liters: number; kmInitial?: number; kmFinal?: number; notes?: string }) {
  ...
  if (currentUser.role === 'driver') {
    const [driver] = await db.select({ assignedTruckId: drivers.assignedTruckId }).from(drivers).where(eq(drivers.userId, currentUser.id)).limit(1)
    if (!driver?.assignedTruckId || driver.assignedTruckId !== input.truckId) throw new Error('Você só pode registrar no caminhão atribuído ao seu cadastro.')
  }
  if (!Number.isFinite(input.kmInitial) || !Number.isFinite(input.kmFinal) || input.kmFinal! < input.kmInitial!) {
    throw new Error('O KM final deve ser maior ou igual ao KM inicial.')
  }
  if (!Number.isFinite(input.trips) || input.trips <= 0) throw new Error('Informe pelo menos uma viagem.')
...
export async function createFuelRecord(input: { truckId?: number; driverName?: string; recordDate?: string; km: number; liters: number; pricePerLiter?: number; station?: string }) {
  ...
  if (currentUser.role === 'driver') {
    const [driver] = await db.select({ assignedTruckId: drivers.assignedTruckId }).from(drivers).where(eq(drivers.userId, currentUser.id)).limit(1)
    if (!driver?.assignedTruckId || driver.assignedTruckId !== input.truckId) throw new Error('Você só pode abastecer o caminhão atribuído ao seu cadastro.')
  }
