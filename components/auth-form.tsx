...
export function AuthForm({ mode }: { mode: 'sign-in' | 'sign-up' }) {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); setPending(true); setError(''); const result = mode === 'sign-up' ? await authClient.signUp.email({ name, email, password }) : await authClient.signIn.email({ email, password }); setPending(false); if (result.error) { setError('Não foi possível concluir. Verifique os dados e tente novamente.'); return } router.push('/'); router.refresh() }
